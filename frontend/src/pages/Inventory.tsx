...
const EMPTY_FORM: AssetRequest = { name: '', software: '', ecosystem: '', version: '', criticality: 'MEDIA', description: '' }
...
export default function Inventory() {
  ...
  const openEdit   = (a: Asset) => { setEditing(a); setForm({ name: a.name, software: a.software, ecosystem: a.ecosystem, version: a.version, criticality: a.criticality, description: a.description ?? '' }); setModal(true) }
  ...
  const f = (k: keyof AssetRequest, v: string) => setForm(prev => ({ ...prev, [k]: v }))
      ...
      <Modal title={editing ? 'Editar Activo' : 'Nuevo Activo'} open={modal} onClose={() => setModal(false)}>
        <div className="space-y-3">
          ...
          <div>
            ...
            <select className="input" value={form.criticality} onChange={e => f('criticality', e.target.value)}>
              {['CRITICA','ALTA','MEDIA','BAJA'].map(c => <option key={c}>{c}</option>)}
            </select>
          </div>
          <div>
