...
- **`DashboardService`** — agregación de MTTR vía queries de repositorio (`findAverageMttrDeclaredDays[ByPriority]`), aging buckets, breakdowns por severidad/estado/responsable.
  - **Gap conocido**: `mttrVerifiedDays` es un stub (devuelve `null`) — solo `mttrDeclaredDays` está implementado. El MTTR "verificado" (tiempo real hasta que se confirma efectivamente resuelto, no solo declarado por el analista) queda pendiente. No hay ADR para esto porque no es una decisión con alternativas descartadas, es trabajo no terminado — ver `knowledge-base/10_preguntas_abiertas.md` una vez generada.
